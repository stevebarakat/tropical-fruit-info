						...
						datasets: [
							{
								...
								borderWidth: 1,
								hoverBackgroundColor: 'hsla(10, 100%, 50%, .5)',
								hoverBorderColor: 'hsl(10, 100%, 50%)',
								data: nutrientVal,
							}
						]
					},
  ...
  const drawer = (
		<div>
          ...
          <Link to={text.replace(/\s+/g, '-').toLowerCase()}>
						<ListItemText className={classes.sidebarLink} disableTypography={true} primary={text} />
          </Link>
					</ListItem>
				))}
			</List>
    ...
    const urls = [
      ...
      'https://api.nal.usda.gov/ndb/V2/reports?ndbno=16001&type=b&format=json&api_key=VEXk4M7U4EvgfugX5L0QtdbtnoLO6szde9XWqMGi'
    ]
		async function getAllUrls(urls) {
      setFetching(true)
			try {
